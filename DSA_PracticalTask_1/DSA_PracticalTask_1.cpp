...
void printPolynomial(term *p, char c)
{
   // Вывод на консоль имени полинома и его первого члена. Первый член выводится вне цикла, чтобы упростить общую логику вывода остальных членов полинома (знак '+' не должен выводится у первого члена, что возможно в логике вывода в цикле).
   printf_s("%cn(x) = %dx^%d", c, p->a, p->n);
   for (p = p->next; p; p = p->next)
   {
      // Вывод части члена полинома вида " + a" или " - а", в зависимости от знака коэффициента.
